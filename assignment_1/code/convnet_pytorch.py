...
class ConvNet(nn.Module):
  ...
  def forward(self, x):
    ...
    x = F.relu(self.conv3a_bn(self.conv3_a(x)))
    x = F.relu(self.conv3b_bn(self.conv3_b(x)))
    x = self.maxpool3(x)

    x = F.relu(self.conv4a_bn(self.conv4_a(x)))
    x = F.relu(self.conv4b_bn(self.conv4_b(x)))
    x = self.maxpool4(x)

    x = F.relu(self.conv5a_bn(self.conv5_a(x)))
    x = F.relu(self.conv5b_bn(self.conv5_b(x)))
    x = self.maxpool5(x)
